...
function rotateDegree(prize) {
  ...
  if (prize == 1) {
    ...
    deg +=
      no_prize_items[Math.floor(Math.random() * no_prize_items.length)] +
      randomDeg;
  }
  return deg;
}
...
$(document).ready(function() {
  ...
  // prize = prizeID;
  // console.log(prize);
  //   $("#prizeID").val(); //todo: Raphael change this dynamic
  getResult();
    ...
    timer = setInterval(function() {
      ...
      setTimeout(function() {
        ...
        if (prize == 1) {
          ...
          window.location.href = "../prize/2";
        } else if (prize == 3) {
          // Wine stopper/opener set 奔馳開瓶器/酒塞 （0.3%）
          window.location.href = "../prize/3";
        } else if (prize == 4) {
          // Smart brand silicone mug  Smart馬克杯 （0.3%）
          window.location.href = "../prize/4";
        } else if (prize == 5) {
          // Metal water bottle 奔馳金屬質水壺 （0.2%）
          window.location.href = "../prize/5";
        } else if (prize == 6) {
          // Golf ball, tees, towel set 奔馳高爾夫球/球釘/毛巾三件套 （0.1%）
          window.location.href = "../prize/6";
        } else if (prize == 7) {
          //未中奖
          window.location.href = "../noprize";
        }
      }, 4000);
    }, 30);
  }
